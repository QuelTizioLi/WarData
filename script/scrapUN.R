...
if (!nrow(data)) {
  ...
  for (i in 1:n) {
    res$Civilian[i] <-
      as.numeric(str_extract(data$text[i], "[0-9]+(?=\\s*civilian)"))
    res$KilledTotal[i]  <-
      as.numeric(str_extract(data$text[i], "[0-9]+(?=\\s*killed)"))
    res$KilledChildren[i] <-
      as.numeric(str_extract(data$text[i], "[0-9]+(?=\\s*children;)"))
    res$InjuredTotal[i] <-
      as.numeric(str_extract(data$text[i], "[0-9]+(?=\\s*injured)"))
    res$InjuredChildren[i] <-
      as.numeric(str_extract(data$text[i], "[0-9]+(?=\\s*children )"))
    ...
    res$Date[i] <-
      as.character(as.Date(tempDate, format = '%d %b %Y'))
